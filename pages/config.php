...
$n = [];
$n['label'] = '<label for="rex-gpimporter-config-url">' . $this->i18n('config_url') . '</label>';
$n['field'] = '<input type="text" id="rex-gpimporter-config-url" name="config[url]" value="' . $this->getConfig('url') . '"/>';
$formElements[] = $n;

$n = [];
$n['label'] = '<label for="rex-gpimporter-config-ids">' . $this->i18n('config_ids') . '</label>';
$select = new rex_select();
$select->setId('rex-gpimporter-config-ids');
$select->setMultiple();
